...
`top` 명령어는 Unix 및 Linux 시스템에서 실행 중인 프로세스들의 현재 상태를 실시간으로 모니터링할 수 있는 명령줄 도구입니다. 주로 시스템 관리자나 개발자들이 시스템 성능을 모니터링하거나 문제 해결에 사용합니다. 다음은 `top` 명령어의 간단한 설명입니다:

1. **실시간 업데이트**: `top`은 기본적으로 화면을 지속적으로 업데이트하여 실행 중인 프로세스 목록을 실시간으로 보여줍니다.
    
2. **프로세스 목록**: `top`은 실행 중인 모든 프로세스의 목록을 제공합니다. 각 프로세스에는 PID (프로세스 식별자), 사용자, CPU 및 메모리 사용량, 실행 시간 등의 정보가 포함될 수 있습니다.
    
3. **리소스 사용량**: `top`은 CPU 및 메모리 사용량에 대한 정보를 제공합니다. 이 정보는 프로세스 단위로 표시되며, 사용량이 가장 높은 프로세스가 상단에 표시됩니다.
    
4. **대화형 인터페이스**: `top`은 상호 작용적인 명령어를 제공하여 사용자가 정렬 기준을 변경하거나 프로세스를 종료하는 등의 작업을 수행할 수 있습니다.
    
5. **정렬 및 필터링**: `top`은 프로세스 목록을 다양한 기준으로 정렬하고 필터링할 수 있습니다. CPU 사용량, 메모리 사용량, 실행 시간 등을 기준으로 정렬할 수 있습니다.
    

`top` 명령어는 많은 옵션을 제공하며, 다양한 시스템 관리 작업을 수행할 수 있습니다. 일반적으로는 기본적인 사용법만으로도 대부분의 요구 사항을 충족할 수 있습니다. 페이지 업/다운 키를 사용하여 스크롤하고, q를 눌러 종료할 수 있습니다. 자세한 사용법은 `man top`을 통해 확인할 수 있습니다.

1. **-c**: 명령어 행에 대한 명령어 이름을 표시합니다.
2. **-d <초>**: 화면 갱신 속도를 지정합니다. 기본적으로 5초마다 업데이트됩니다.
3. **-n <횟수>**: `top`을 몇 번만 실행할 것인지 지정합니다.
4. **-p <PID 리스트>**: 특정 PID를 지정하여 해당 프로세스만 표시합니다.
5. **-u <사용자명>**: 특정 사용자의 프로세스만 표시합니다.
6. **-U <사용자명>**: 모든 사용자를 나열하지만 지정된 사용자의 프로세스만 강조 표시합니다.
7. **-o <필드>**: 특정 필드를 기준으로 정렬합니다. 예를 들어, `%CPU` 또는 `%MEM`을 사용할 수 있습니다.
8. **-h**: 도움말을 표시합니다.
9. -b: - 순간의 정보를 확인하려면 `-b` 옵션 추가(batch 모드)

프로세스 목록을 필터링하려면 `-p` 옵션을 사용하여 원하는 PID를 지정하면 됩니다. 예를 들어, PID가 1234인 프로세스만 보려면 다음과 같이 명령어를 실행합니다:

```bash
top -p 1234
```

여러 PID를 지정하려면 쉼표로 구분합니다. 예를 들어, PID가 1234와 5678인 프로세스를 모두 보려면:
```bash
top -p 1234,5678
```
