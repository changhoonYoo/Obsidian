...
4. **커서 이동:**
    ...
    - `k`: 위로 커서 이동
    - `l`: 오른쪽으로 커서 이동
    -  w, b : w키를 누르면 한 단어 오른쪽으로 이동하고, b키를 누르면 한 단어 왼쪽으로 이동합니다. 커서는 항상 단어의 첫 글자를 가리킵니다.  
5. **삭제 및 복사 붙여넣기:**
...
`   chmod` 명령어는 파일의 권한을 변경하는 데 사용됩니다. 이 명령어는 파일이나 디렉터리에 대한 읽기, 쓰기 및 실행 권한을 설정할 수 있습니다. `chmod` 명령어는 다음과 같은 형식으로 사용됩니다.

```bash
chmod [옵션] 권한 파일이름
```

여기서 `옵션`은 권한을 설정하는 방법을 지정하는 데 사용됩니다. 일반적으로 사용되는 옵션은 다음과 같습니다.

- `-R` 또는 `--recursive`: 지정된 디렉터리와 그 하위 디렉터리에 대해 재귀적으로 권한을 설정합니다.
- `-v` 또는 `--verbose`: 작업의 결과를 자세하게 출력합니다.

`권한`은 파일 또는 디렉터리에 대한 권한을 나타내며, 다음과 같은 형식으로 지정됩니다.

- `u`는 파일 소유자에 대한 권한을 나타냅니다.
- `g`는 파일 소유 그룹에 대한 권한을 나타냅니다.
- `o`는 다른 사용자에 대한 권한을 나타냅니다.
- `a`는 모든 사용자에 대한 권한을 나타냅니다.

`권한`은 다음과 같은 형식으로 지정됩니다.
...
`df` 명령어는 "disk free"의 약자로, 파일 시스템의 디스크 여유 공간을 확인하는 명령어입니다. `df` 명령어는 다양한 옵션을 사용하여 파일 시스템의 여러 측면을 확인할 수 있습니다. 주요 옵션은 다음과 같습니다:

- `-h`: 사람이 읽기 쉬운 형식으로 출력합니다. 파일 시스템의 크기와 사용 중인 공간, 사용 가능한 공간 등을 KB, MB, GB 등의 단위로 표시합니다.
- `-T`: 파일 시스템 유형을 표시합니다.
- `-t <타입>`: 특정 파일 시스템 유형만 표시합니다.
- `-i`: 파일 시스템의 inode 사용 현황을 표시합니다.
- `--total`: 모든 파일 시스템에 대한 합계 행을 표시합니다.
- `--output=<필드>`: 원하는 필드만 선택하여 출력합니다. 여러 필드를 선택할 수도 있습니다.
