...
스프링 MVC는 널리 사용되어 온 자바 서블릿 API를 기반으로 만들어졌다. 서블릿 API는 그 규약 내부적으로 많은 가정에 의존하고 있다. 그 중 가장 큰 부분을 차지하는 것은 서블릿 API는 블로킹 방식으로 동작한다는 점이다. 물론 서블릿 3.1 이래로 비동기 방식을 일부 지원하기 시작했지만 완전히 리액티브하다고 볼 수는 없다. 서블릿 3.1에